---
layout: page
permalink: /Services/
title: Services
description: 
nav: true
nav_order: 30
---


<article>


<h3 id="reviewer">Reviewer</h3>
<ul>
  <li>IEEE Transactions on Circuits and Systems (TCAS), 2025-present</li>
  <li>Springer The Journal of Supercomputing (TJSC), 2024-present</li>
  <li>IEEE Transactions on Vehicular Technology (TVT), 2023-present</li>
  <li>IEEE Transactions on Mobile Computing (TMC), 2021-present</li>
</ul>

<h3 id="teaching">Teaching</h3>
<ul>

  <li>
<strong>Teaching Assistant</strong>: The Frontiers of Computer Networking (Graduate Course)
    <ul>
      <li>Spring 2024, Tsinghua University</li>
    </ul>
  </li>

  <li>
<strong>Teaching Assistant</strong>: Systems Thinking and Systems Engineering (Undergraduate Course)
    <ul>
      <li>Fall 2024, Tsinghua University</li>
    </ul>
  </li>

</ul>


</article>